            ...
            <div className={`grid grid-cols-1 ${cat.services.length === 1 ? "md:grid-cols-1 max-w-3xl" : cat.services.length === 2 ? "md:grid-cols-2" : "md:grid-cols-2 lg:grid-cols-3"} gap-6 md:gap-8`}>
              {cat.services.map((service, i) => (
                <AnimatedSection key={service.title} delay={i * 0.1}>
                 <Link to={service.href} className="block p-8 md:p-10 rounded-3xl h-full flex flex-col group bg-primary/[0.08] border border-primary/30 hover:bg-primary/[0.12] hover:border-primary/50 hover:-translate-y-1.5 hover:shadow-[0_30px_80px_-20px_hsl(var(--primary)/0.4)] transition-all duration-500">
                   <div className="w-11 h-11 md:w-12 md:h-12 rounded-2xl bg-primary/25 border border-primary/40 flex items-center justify-center mb-5 md:mb-6 group-hover:bg-primary/35 group-hover:scale-110 transition-all duration-500">
