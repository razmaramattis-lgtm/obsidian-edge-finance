...
import { ArrowRight, Shield, Users, Monitor, Calculator, TrendingUp, Handshake, CheckCircle2, Building2, Megaphone, Sparkles } from "lucide-react";
...
const SamarbeidForside = () => {
  const [kartleggingOpen, setKartleggingOpen] = useState(false);
  ...
  return (
    <>
      ...
      <KartleggingDialog open={kartleggingOpen} onOpenChange={setKartleggingOpen} />
        ...
        <div className="container mx-auto px-4 relative z-10 py-20">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="max-w-2xl">
            ...
            <div className="flex flex-wrap gap-3">
              <button onClick={() => setKartleggingOpen(true)} className="inline-flex items-center gap-2 px-7 py-3.5 bg-primary text-primary-foreground rounded-xl text-sm font-semibold hover:opacity-90 transition-all shadow-lg shadow-primary/30">
      ...
      <section className="relative py-20 md:py-28 overflow-hidden">
        ...
        <div className="container mx-auto px-4 relative z-10 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            ...
            <p className="text-lg text-white/60 max-w-lg mx-auto mb-8">Start en uforpliktende kartlegging — vi tar kontakt for en konfidensiell samtale.</p>
            <button onClick={() => setKartleggingOpen(true)} className="inline-flex items-center gap-2 px-8 py-4 bg-primary text-primary-foreground rounded-xl text-sm font-semibold hover:opacity-90 transition-all shadow-lg shadow-primary/30">
              <Sparkles size={15} /> Start kartlegging
            </button>
          </motion.div>
        </div>
