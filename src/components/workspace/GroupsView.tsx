...
const GroupsView = ({ profile, onViewProfile, onComposingChange, initialGroupId }: { profile: Profile; onViewProfile?: (p: Profile) => void; onComposingChange?: (c: boolean) => void; initialGroupId?: string }) => {
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden">
      {!active ? (
        <>
          <div className="px-6 py-5 border-b border-border/10 bg-card/20 shrink-0 flex items-center justify-between">
            <div>
        ...
        <div className="flex-1 flex flex-col min-w-0 min-h-0 overflow-hidden">
          ...
          {/* Group tabs */}
          <div className="flex items-center gap-1 px-4 py-2 border-b border-border/10 bg-card/20 shrink-0 overflow-x-auto">
            {([
