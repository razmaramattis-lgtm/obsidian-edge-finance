...
import {
  ...
  Subscript as SubscriptIcon, Palette, Pilcrow, RemoveFormatting, TableIcon
} from "lucide-react";
...
const Divider = () => <div className="w-px h-6 bg-border/30 mx-1 self-center" />;
...
const RichTextEditor = ({ content, onChange, onImageUpload, placeholder = "Skriv innholdet her…" }: RichTextEditorProps) => {
  ...
  const editor = useEditor({
    extensions: [
      ...
      Image.configure({
        HTMLAttributes: { class: "rounded-xl max-w-full my-8 mx-auto block shadow-sm" },
        allowBase64: true,
      }),
      ...
      Color,
      Youtube.configure({
        HTMLAttributes: { class: "rounded-xl overflow-hidden my-8 w-full aspect-video" },
        width: 0,
        height: 0,
      }),
      Superscript,
    ...
    editorProps: {
      attributes: {
        class: "article-content min-h-[500px] px-8 py-6 focus:outline-none",
      },
    },
  });
  ...
  const insertTable = useCallback(() => {
    if (!editor) return;
    const html = `<table><thead><tr><th>Kolonne 1</th><th>Kolonne 2</th><th>Kolonne 3</th></tr></thead><tbody><tr><td>Data</td><td>Data</td><td>Data</td></tr><tr><td>Data</td><td>Data</td><td>Data</td></tr></tbody></table>`;
  ...
  return (
    <div className="rounded-2xl border border-border/30 bg-background overflow-hidden shadow-sm">
      {/* Toolbar Row 1 - Block types & Text formatting */}
      <div className="flex flex-wrap items-center gap-0.5 px-3 py-2 border-b border-border/20 bg-muted/20">
        {/* Heading buttons with visual size indicators */}
        <MenuButton onClick={() => editor.chain().focus().toggleHeading({ level: 1 }).run()} active={editor.isActive("heading", { level: 1 })} title="Overskrift 1 (Hovedoverskrift)">
          <span className="text-[13px] font-black leading-none">H1</span>
        </MenuButton>
        <MenuButton onClick={() => editor.chain().focus().toggleHeading({ level: 2 }).run()} active={editor.isActive("heading", { level: 2 })} title="Overskrift 2 (Kapitteltittel)">
          <span className="text-[12px] font-bold leading-none">H2</span>
        </MenuButton>
        <MenuButton onClick={() => editor.chain().focus().toggleHeading({ level: 3 }).run()} active={editor.isActive("heading", { level: 3 })} title="Overskrift 3 (Undertittel)">
          <span className="text-[11px] font-semibold leading-none">H3</span>
        </MenuButton>
        <MenuButton onClick={() => editor.chain().focus().toggleHeading({ level: 4 }).run()} active={editor.isActive("heading", { level: 4 })} title="Overskrift 4 (Liten tittel)">
          <span className="text-[10px] font-medium leading-none">H4</span>
        </MenuButton>
        <MenuButton onClick={() => editor.chain().focus().setParagraph().run()} active={editor.isActive("paragraph")} title="Brødtekst"><Pilcrow size={15} /></MenuButton>
      ...
      {/* Toolbar Row 2 - Structure, Lists, Media */}
      <div className="flex flex-wrap items-center gap-0.5 px-3 py-2 border-b border-border/20 bg-muted/10">
        ...
        <MenuButton onClick={() => editor.chain().focus().setHorizontalRule().run()} title="Skillelinje"><Minus size={15} /></MenuButton>
        <MenuButton onClick={insertTable} title="Sett inn tabell"><TableIcon size={15} /></MenuButton>
        ...
        <div className="ml-auto text-[10px] text-muted-foreground/50 pr-2 font-mono">
          {editor.storage.characterCount?.characters?.() ?? editor.getText().length} tegn
        </div>
      </div>

      {/* Editor Area — styled like a newspaper page */}
