...
const SmsCampaignsPanel = () => {
  ...
  const handleCreate = async () => {
    ...
    if (error || !campaign) {
      toast.error("Feil: " + (error?.message || "Ukjent"));
      return;
    }

    // Insert campaign contacts
    if (phones.length > 0) {
      await supabase.from("sms_campaign_contacts").insert(
        phones.map(phone => ({ campaign_id: campaign.id, phone }))
      );
    }
  ...
  const handleStart = async (id: string) => {
    // Get campaign contacts and queue messages
    ...
    if (!contacts || contacts.length === 0) {
      toast.error("Ingen kontakter i kampanjen");
      return;
    }
              ...
              <TableCell>
                <div className="flex gap-1">
                  {(c.status === "draft" || c.status === "scheduled") && (
                    <Button size="icon" variant="ghost" onClick={() => handleStart(c.id)} title="Start sending">
                      <Play size={14} />
                    </Button>
                  )}
