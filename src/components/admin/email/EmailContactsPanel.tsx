...
const EmailContactsPanel = () => {
  ...
  const fetchAll = async () => {
    const [cRes, gRes] = await Promise.all([
      ...
      supabase.from("email_contact_groups").select("*").order("name"),
    ]);
  ...
  const handleAdd = async () => {
    if (!form.name.trim() || !form.email.trim()) { toast.error("Navn og e-post er påkrevd"); return; }
  ...
  const handleAddGroup = async () => {
    ...
    await supabase.from("email_contact_groups").insert({ name: groupForm.name.trim(), description: groupForm.description.trim() || null });
  ...
  const handleDeleteGroup = async (id: string) => {
    await supabase.from("email_contact_groups").delete().eq("id", id);
    fetchAll();
  ...
  const handleCsvImport = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (ev) => {
      ...
      const rows = lines.map(line => {
        ...
        return { name: cols[0]?.trim() || "Ukjent", email: cols[1]?.trim() || "", tags: [] as string[] };
      }).filter(r => r.email.includes("@"));
      if (rows.length > 0) {
        await supabase.from("email_contacts").insert(rows);
        fetchAll();
      ...
      <Tabs defaultValue="contacts">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-2">
            ...
            <div className="flex gap-2">
              <Dialog open={openContact} onOpenChange={setOpenContact}>
                ...
                <DialogContent>
                  <DialogHeader><DialogTitle>Ny e-postkontakt</DialogTitle></DialogHeader>
                  <div className="space-y-3">
                    <div><Label>Navn</Label><Input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} /></div>
                    <div><Label>E-post</Label><Input type="email" value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} placeholder="navn@eksempel.no" /></div>
                ...
                <CardContent className="p-3">
                  {editId === c.id ? (
                    <div className="space-y-2">
                      ...
                      <Input value={editForm.email} onChange={e => setEditForm(f => ({ ...f, email: e.target.value }))} className="h-8" placeholder="E-post" />
                    ...
                    <div className="space-y-1.5">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex gap-1">
                          <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => { setEditId(c.id); setEditForm({ name: c.name, email: c.email, tags: c.tags?.join(", ") || "" }); }}>
                            <Edit2 size={12} />
                          </Button>
                  ...
                  <TableRow key={c.id}>
                    {editId === c.id ? (
                      <>
                        ...
                        <TableCell><Input value={editForm.email} onChange={e => setEditForm(f => ({ ...f, email: e.target.value }))} className="h-8" /></TableCell>
                      ...
                      <>
                        ...
                        <TableCell>
                          <div className="flex gap-1">
                            <Button size="icon" variant="ghost" onClick={() => { setEditId(c.id); setEditForm({ name: c.name, email: c.email, tags: c.tags?.join(", ") || "" }); }}><Edit2 size={14} /></Button>
        ...
        <TabsContent value="groups" className="space-y-4">
          <Dialog open={openGroup} onOpenChange={setOpenGroup}>
            ...
            <DialogContent>
              <DialogHeader><DialogTitle>Ny e-postgruppe</DialogTitle></DialogHeader>
              <div className="space-y-3">
...
export default EmailContactsPanel;
