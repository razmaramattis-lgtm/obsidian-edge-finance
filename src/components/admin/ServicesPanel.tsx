  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <p className="text-muted-foreground text-sm">{items.length} tjenester · Endringer påvirker fremsiden</p>
      ...
      {groups.map(group => (
        <div key={group}>
          ...
          <div className="space-y-2">
            {items.filter(i => i.group_name === group).map(item => (
              <div key={item.id} className="glass rounded-2xl px-5 py-4 border border-border/20 flex items-center justify-between gap-4">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground truncate">{item.href || "Ingen lenke"}</p>
                  </div>
                </div>
                <div className="flex gap-2 shrink-0">
                  {item.href && (
                    <a href={item.href} target="_blank" rel="noreferrer" className="text-muted-foreground hover:text-primary">
