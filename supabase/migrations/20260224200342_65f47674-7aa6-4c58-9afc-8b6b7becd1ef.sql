-- Remove workspace notification triggers for job/open applications
-- (admin dashboard handles these via direct queries, no need for workspace_notifications)
DROP TRIGGER IF EXISTS notify_on_job_application ON public.job_applications;
DROP TRIGGER IF EXISTS notify_on_open_application ON public.open_applications;

-- Update the function to remove these cases
